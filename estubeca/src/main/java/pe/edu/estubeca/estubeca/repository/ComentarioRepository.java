...
public interface  ComentarioRepository extends JpaRepository<Comentario, Long> {
    ...
    @Query(value = "SELECT * FROM comentarios c inner join posts p on c.post_id = p.id WHERE p.id=?1", nativeQuery = true)
    List<Comentario> findAllComentariosPostIdSQL(Long postId);
